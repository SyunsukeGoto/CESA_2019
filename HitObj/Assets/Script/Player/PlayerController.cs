...
namespace Momoya
{
    public class PlayerController : MonoBehaviour
    {
        ...
        //変数の宣言
        [SerializeField]
        GameObject[] _doNotTime = new GameObject[3];
        CarContoller _carContoller;             //動けない時間
        SpiderWeb _spiderWeb;                   //動けない時間
        BicycleContoller _bicycleContoller;     //動けない時間
        ...
        void Start()
        {
            ...
            //スクリプトのゲット
            _carContoller = _doNotTime[0].GetComponent<CarContoller>();
            _spiderWeb = _doNotTime[1].GetComponent<SpiderWeb>();
            _bicycleContoller = _doNotTime[2].GetComponent<BicycleContoller>();

            Debug.Log(_spiderWeb);
            Debug.Log(_carContoller);
            Debug.Log(_bicycleContoller);
        }
